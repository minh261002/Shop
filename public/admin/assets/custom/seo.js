(function($) {
    ...
    ELEMENT.seoPreview = () => {
       ...
       $('input[name=canonical]').on('keyup', function(){
            let input = $(this)
            let value = ELEMENT.removeUtf8(input.val())
            $('.canonical').text(BASE_URL + value + SUFFIX)
        })
        ...
        $(document).on('keyup', '.change-title', function(e){
            ...
            let canonical = ELEMENT.removeUtf8(_this.val())
            if(flag == 0){
    ...
    ELEMENT.checkSeoDescriptionLength = () => {
