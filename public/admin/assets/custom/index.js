                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if(res.flag == true){
                            ...
                            let cssActive2 = ' background-color: rgb(255, 255, 255); transition: background-color 0.4s ease 0s, left 0.2s ease 0s;';
    ...
    ELEMENT.checkAll = () => {
        if($('#checkAll').length){
            $(document).on('click', '#checkAll', function(){
                ...
                $('.checkBoxItem').each(function(){
                    let _this = $(this)
                    ELEMENT.changeBackground(_this)
                })
            })
        }
    }
    ...
    ELEMENT.checkBoxItem = () => {
        if($('.checkBoxItem').length){
            $(document).on('click', '.checkBoxItem', function(){
                let _this = $(this)
                ELEMENT.changeBackground($(this))
                ELEMENT.allChecked()
            })
        }
    }

    ELEMENT.changeBackground = (object) => {
